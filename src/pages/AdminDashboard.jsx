...
export default function AdminDashboard({ navigate }) {
  ...
  const filteredProspects = useMemo(() => {
    ...
    return prospects.filter(p => {
      ...
      const matchesSent = prospectSentFilter === 'all' || (prospectSentFilter === 'pending' ? !p.emailSent && !['descartado', 'error_envio'].includes(p.emailStatus) : prospectSentFilter === 'error' ? p.emailStatus === 'error_envio' : prospectSentFilter === 'discarded' ? p.emailStatus === 'descartado' : p.emailSent);
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                <input
                  ...
                  checked={filteredProspects.filter(p => !p.emailSent && p.emailStatus !== 'descartado').length > 0 && filteredProspects.filter(p => !p.emailSent && p.emailStatus !== 'descartado').every(p => selectedProspectIds.includes(p.id))}
                  ...
                  style={{ marginTop: '4px' }}
                />
              <div><h3 style={{ margin: 0 }}>3. Contactos importados ({uniqueProspectEmails} correos únicos)</h3><span style={{ color: '#64748b', fontSize: '12px' }}>Mostrando {filteredProspects.length} de {uniqueProspectEmails} · {prospects.filter(p => !p.emailSent && !['descartado', 'error_envio'].includes(p.emailStatus)).length} pendientes · {prospects.filter(p => p.emailSent).length} enviados · {prospects.filter(p => p.emailStatus === 'error_envio').length} con error · {prospects.filter(p => p.emailStatus === 'descartado').length} descartados</span></div>
              </div>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                ...
                <select value={prospectSentFilter} onChange={e => setProspectSentFilter(e.target.value)} style={{ padding: '9px', border: '1px solid #cbd5e1', borderRadius: '8px' }}><option value="pending">Pendientes</option><option value="error">Errores de envío</option><option value="sent">Enviados</option><option value="discarded">Descartados</option><option value="all">Todos</option></select>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap' }}>
              <button type="button" className="btn btn-secondary" onClick={() => setSelectedProspectIds(filteredProspects.filter(p => !p.emailSent && p.emailStatus !== 'descartado').map(p => p.id))}>Seleccionar contactos disponibles</button>
