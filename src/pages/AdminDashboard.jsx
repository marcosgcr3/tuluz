...
export default function AdminDashboard({ navigate }) {
  ...
  const filteredProspects = useMemo(() => {
    ...
    return prospects.filter(p => {
      ...
      const matchesSent = prospectSentFilter === 'all' || (prospectSentFilter === 'pending' ? !p.emailSent && p.emailStatus !== 'descartado' : prospectSentFilter === 'discarded' ? p.emailStatus === 'descartado' : p.emailSent);
          ...
          <section style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '14px', padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                ...
                <select value={prospectSentFilter} onChange={e => setProspectSentFilter(e.target.value)} style={{ padding: '9px', border: '1px solid #cbd5e1', borderRadius: '8px' }}><option value="pending">Pendientes</option><option value="sent">Enviados</option><option value="discarded">Descartados</option><option value="all">Todos</option></select>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap' }}>
              <button type="button" className="btn btn-secondary" onClick={() => setSelectedProspectIds(filteredProspects.filter(p => !p.emailSent && p.emailStatus !== 'descartado').map(p => p.id))}>Seleccionar pendientes visibles</button>
